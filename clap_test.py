...
if __name__ == "__main__":
    ...
    wt_trace_univariate = wt_trace.get_columns(["rain"])  # Replace with actual column names
    ###################################################### CLASP segmentation ######################################################
    ...
    ##3-WT##
    clasp = BinaryClaSPSegmentation(validation="score_threshold", threshold=0.70)
    clasp.fit(wt_trace_multivariate)
    clasp.predict()
    change_points = clasp.change_points.tolist()
    print("Detected change points:", change_points)
    # Plotting the segmentation results
    clasp.plot(heading="Segmentation of WT signals", ts_name="ACC", file_path="WT_segmentation.png")
    ####################################
    
    # ######### UNIVARIATE ###############
    # clasp = BinaryClaSPSegmentation()
    # clasp.fit_predict(Signal.reshape(-1, 1))
    
    # ## Plotting the segmentation results
    # clasp.plot(heading="Segmentation of a test signal", ts_name="ACC", file_path="Amplitude_example.png")
    
    ##3-WT Univariate##
    clasp = BinaryClaSPSegmentation(validation="score_threshold", threshold=0.62)
    clasp.fit(wt_trace_univariate)
    clasp.predict()
    change_points = clasp.change_points.tolist()
    print("Detected change points (univariate):", change_points)
    ## Plotting the segmentation results
    clasp.plot(heading="Segmentation of WT signals", ts_name="ACC", file_path="WT_segmentation_univariate.png")